...
import {DebugMessage, PendingMessage, TextMessage,} from './messages';
...
export class AgentView extends ItemView {
    ...
    handleUpdate(update: SessionUpdate): void {
        ...
        if (update.type === 'permission_request') {
            ...
            this.handlePermissionRequest(update.data.params, update.data.resolve);
            return;
        }
    ...
    private handlePermissionRequest(params: RequestPermissionRequest, resolve: (response: RequestPermissionResponse) => void): void {
        this.messageRenderer.endCurrentAgentMessage();
        this.messageRenderer.endCurrentThoughtMessage();
        ...
        void this.messageRenderer.applyPermissionToToolCall(
            toolCallId,
            params.options,
            wrappedResolve
        );

        new Notice('Approval required for tool call');
    }
    ...
    approvePermission(): void {
    	// Try to find an allow button (allow_once or allow_always)
    	const allowBtn = this.messagesContainer.querySelector('.acp-tool-permission-allow_once, .acp-tool-permission-allow_always') as HTMLElement;
    	if (allowBtn) {
    		allowBtn.click();
    	}
    }
    ...
    rejectPermission(): void {
    	// Try to find a reject button (reject_once or reject_always)
    	const rejectBtn = this.messagesContainer.querySelector('.acp-tool-permission-reject_once, .acp-tool-permission-reject_always') as HTMLElement;
    	if (rejectBtn) {
    		rejectBtn.click();
    	}
    }
